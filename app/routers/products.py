...
async def create_product(db: Annotated[AsyncSession, Depends(get_db)], create_products: CreateProduct,
                         get_user: Annotated[dict, Depends(get_current_user)]):
    if get_user.get('is_admin') or get_user.get('is_supplier'):
        category = await db.scalar(select(Category).where(Category.id == create_products.category))
        if category is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail='There is no category found'
            )
        await db.execute(insert(Product).values(name=create_products.name,
                                                description=create_products.description,
                                                price=create_products.price,
                                                image_url=create_products.image_url,
                                                stock=create_products.stock,
                                                category_id=create_products.category,
                                                supplier_id=get_user.get('id'),
                                                rating=0.0,
                                                slug=slugify(create_products.name)))
        await db.commit()
        return {
